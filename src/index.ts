  ...
  rl.question("You: ", async (input) => {
    ...
    try {
      const { text } = await generateText({
        ...
        system: "You are an onchain AI agent. You have tools to interact with the blockchain and make HTTP requests. Always use your tools — never say you can't check balances, send transactions, or fetch URLs.",
        tools,
        maxSteps: 10,
        prompt: message,
      });
