...
if (balance) {
  ...
  const toolResults = steps1.flatMap(s => (s as any).toolResults ?? []) as any[];
  toolResults.forEach(r => console.log("Result:", JSON.stringify(r.result, null, 2)));
  console.log();
}
...
if (sent) {
  ...
  const toolResults = steps2.flatMap(s => (s as any).toolResults ?? []) as any[];
  toolResults.forEach(r => console.log("Result:", JSON.stringify(r.result, null, 2)));
  console.log();
}
console.log(
